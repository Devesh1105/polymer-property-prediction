...
The current reference point for this property set is **PolyCL** (Zhang et al., *RSC Digital Discovery* 2025), which reports R² directly — so the comparison needs no metric conversion. PolyCL benchmarks itself against PolyBERT, TransPolymer and GPPG; the "best prior" column below is the strongest of those three on each property, taken from PolyCL's own table.

| Property | PolyBERT / TransPolymer / GPPG — best prior | PolyCL | **This work** | vs PolyCL | vs best prior |
|---|---|---|---|---|---|
| `Egc` | 0.9000 *(GPPG)* | 0.8832 | **0.9239** | **+0.041** | **+0.024** |
| `Egb` | 0.9100 *(GPPG)* | 0.8884 | **0.9506** | **+0.062** | **+0.041** |
| `Ei` | 0.7670 *(PolyBERT)* | 0.8112 | **0.8771** | **+0.066** | **+0.110** |
| `Eea` | 0.8943 *(TransPolymer)* | 0.9071 | **0.9458** | **+0.039** | **+0.052** |
| `EPS` | 0.7694 *(PolyBERT)* | 0.7876 | **0.8735** | **+0.086** | **+0.104** |
| `Nc` | 0.8017 *(PolyBERT)* | 0.8460 | **0.9318** | **+0.086** | **+0.130** |
| **mean (6 shared)** | **0.8404** | **0.8539** | **0.9171** | **+0.063** | **+0.077** |

**Ahead of PolyCL on all six shared properties, and ahead of every baseline PolyCL was compared against, on all six** — by +0.063 mean R² over PolyCL and +0.077 over the best prior result per property. `Tg` (0.9217 here) is not in PolyCL's benchmark; `Xc`, which is, is not in this competition.

**Read this with its caveats, because they matter.** The two studies use different splits and different subsets of the underlying Ramprasad property data — PolyCL uses 5-fold CV on its own sets, this notebook uses grouped 8-fold CV on the competition release — so this compares *reported numbers*, not a controlled head-to-head. And part of the margin on `Ei`, `Eea`, `EPS` and `Nc` comes from the cross-property channel described below, which reads sibling property values from the training table; that is legitimate information here and the leakage controls are enforced in code (`allowed_cols()`), but the pretrained-transformer baselines were single-property models and were never given it.

The claim this table supports: **higher accuracy than the published contrastive- and transformer-pretrained polymer models on every shared property, while additionally being exactly invariant to repeat-unit representation and attributable down to individual atoms** — neither of which those models offer.
...
- The comparison above is against reported numbers under different split protocols, not a re-run of PolyCL on this exact split. A controlled head-to-head would need PolyCL's released weights evaluated on these folds.
...
- Zhang et al., *PolyCL: contrastive learning for polymer representation learning via explicit and implicit augmentations*, RSC Digital Discovery 2025 — [paper](https://pubs.rsc.org/dd/article/4/1/149/846142/PolyCL-contrastive-learning-for-polymer) — the benchmark compared against above
